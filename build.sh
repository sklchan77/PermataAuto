...
build() {
  ...
  ./gradlew $CLEAN permata:$task $abi $app_sfx
  for path in $(ls permata/build/outputs/*/*/permata*.$ext); do
    local version=${path##*permata-}
    version=${version%%-*}
    local dst="$DEST_DIR/permata-auto-${version}${sfx}.$ext"
    mv "$path" "$dst"
    echo "Built $dst"
  done
}
