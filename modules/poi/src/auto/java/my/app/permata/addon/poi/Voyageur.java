...
public class Voyageur extends Poi.Type implements LocationListener {
	...
	public static FutureSupplier<Voyageur> start(Context ctx) {
		...
		return permCheck.then(r -> TextToSpeech.create(ctx, Locale.ENGLISH)).map(tts -> {
			...
			if (a != null) {
				showAlert(a.getContext(), my.app.permata.R.string.err_failed_start_service, err);
			}
		});
	}
	...
	private void loadDb() {
		...
		PoiAddon addon = PermataApplication.get().getAddonManager().getAddon(PoiAddon.class);
		if (addon == null) {
			stop();
			return;
		}
