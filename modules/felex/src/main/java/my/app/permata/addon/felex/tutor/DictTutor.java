package my.app.permata.addon.felex.tutor;
...
import static my.app.permata.R.string.err_no_audio_record_perm;
...
import my.app.permata.addon.felex.dict.WordMatcher;
...
import my.app.utils.voice.SpeechToText;
import my.app.utils.voice.SpeechToTextException;
...
public class DictTutor implements Closeable {
	...
	private void schedule(Runnable r, long delay) {
		run(PermataApplication.get().getHandler().schedule(r, delay));
	}
	...
	private static String getString(@StringRes int resId) {
		return PermataApplication.get().getString(resId);
	}
