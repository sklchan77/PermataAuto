...
public class WebToolBarMediator implements ToolBarView.Mediator {
	...
	public void enable(ToolBarView tb, ActivityFragment f) {
		...
		addButton(tb, my.app.utils.R.drawable.back, v ->
				requireNonNull(b.getWebView()).goBack(), my.app.utils.R.id.tool_bar_back_button, LEFT);
		addButton(tb, R.drawable.clear, v -> t.setText(""), R.id.browser_addr_clear);
		addButton(tb, my.app.permata.R.drawable.bookmark_filled, v ->
				onBookmarksButtonClick(b), my.app.permata.R.id.bookmarks);
		PermataWebView wv = b.getWebView();
	...
	public void setButtonsVisibility(ToolBarView tb, boolean back, boolean forward) {
		tb.findViewById(my.app.utils.R.id.tool_bar_back_button).setVisibility(back ? VISIBLE : GONE);
	...
	private EditText createAddress(ToolBarView tb, WebBrowserFragment f) {
		...
		t.setBackgroundResource(my.app.utils.R.color.tool_bar_edittext_bg);
