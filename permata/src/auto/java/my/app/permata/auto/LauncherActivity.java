...
import static my.app.permata.ui.activity.MainActivityDelegate.INTENT_ACTION_FINISH;
import static my.app.utils.ui.UiUtils.toIntPx;
import static my.app.utils.ui.UiUtils.toPx;
import static my.app.utils.ui.activity.ActivityDelegate.FULLSCREEN_FLAGS;
...
public class LauncherActivity extends AppCompatActivity {
	public static final String INTENT_EXTRA_MODE = "permata.mirror.mode";
	private static final Pref<Supplier<String[]>> AA_LAUNCHER_APPS = Pref.sa("AA_LAUNCHER_APPS",
			() -> new String[]{PermataApplication.get().getPackageName(), "com.android.chrome",
	...
	private static final class AppListView extends RecyclerView {
		...
		public AppListView(@NonNull Context ctx) {
			super(ctx);
			apps = loadAppList();
			animation = AnimationUtils.loadAnimation(ctx, my.app.utils.R.anim.button_press);
		...
		private static class AppInfo {
			private static final AppInfo ADD =
					new AppInfo(PermataApplication.get().getPackageName(), "add", null, null);
			private static final AppInfo BACK =
					new AppInfo(PermataApplication.get().getPackageName(), "back", null, null);
			private static final AppInfo EXIT =
					new AppInfo(PermataApplication.get().getPackageName(), "exit", null, null);
			final String pkg;
			final String name;
			final String label;
			...
			void setAppInfo(AppInfo appInfo) {
				...
				if (appInfo == AppInfo.ADD) {
					...
					if (appInfo instanceof SelectAppInfo sai) {
						Drawable icon;
						if (sai.selected) {
							if (checkedIcon == null) checkedIcon = loadIcon(my.app.utils.R.drawable.check_box);
							icon = checkedIcon;
						} else {
							if (uncheckedIcon == null)
								uncheckedIcon = loadIcon(my.app.utils.R.drawable.check_box_blank);
							icon = uncheckedIcon;
						}
			...
			public void onClick(View v) {
				...
				if (appInfo.equals(AppInfo.EXIT)) {
					...
					try {
						...
						intent.putExtra(INTENT_EXTRA_MODE, PermataApplication.get().getMirroringMode());
