...
import static my.app.permata.media.pref.BrowsableItemPrefs.SORT_BY_DATE;
import static my.app.permata.media.pref.BrowsableItemPrefs.SORT_BY_FILE_NAME;
import static my.app.permata.media.pref.BrowsableItemPrefs.SORT_BY_NAME;
import static my.app.permata.media.pref.BrowsableItemPrefs.SORT_BY_NONE;
import static my.app.permata.media.pref.BrowsableItemPrefs.SORT_BY_RND;
import static my.app.utils.ui.UiUtils.isVisible;
import static my.app.utils.ui.activity.ActivityListener.FRAGMENT_CHANGED;
...
import my.app.permata.addon.PermataToolAddon;
...
import my.app.utils.ui.view.ImageButton;
...
public class ToolBarMediator implements ToolBarView.Mediator.BackTitleFilter {
	...
	public void enable(ToolBarView tb, ActivityFragment f) {
		...
		if ((f instanceof MediaLibFragment) && a.getPrefs().getShowPgUpDownPref(a)) {
			...
			tb.findViewById(my.app.utils.R.id.tool_bar_back_button);
		}

		for (PermataAddon addon : AddonManager.get().getAddons()) {
			if (addon instanceof PermataToolAddon) {
				((PermataToolAddon) addon).contributeTool(this, tb, f);
			}
		}
	...
	public View focusSearch(ToolBarView tb, View focused, int direction) {
		...
		if (direction == FOCUS_UP) {
			...
			Context ctx = tb.getContext();
			if (id == my.app.utils.R.id.tool_bar_back_button) {
