...
import static my.app.permata.media.pref.BrowsableItemPrefs.SORT_MASK_ALL;
import static my.app.permata.media.pref.BrowsableItemPrefs.SORT_MASK_NAME_RND;
import static my.app.permata.util.Utils.getResourceUri;
...
import my.app.permata.media.engine.MetadataRetriever;
...
import my.app.permata.media.pref.StreamItemPrefs;
import my.app.permata.provider.PermataContentProvider;
...
public interface MediaLib {
	...
	@NonNull
	PermataVfsManager getVfsManager();
	...
	interface Item {
		...
		default FutureSupplier<MediaDescriptionCompat> getMediaItemDescription() {
			return getMediaDescription().then(md -> {
				...
				if ((uri != null) && PermataContentProvider.isSupportedFileScheme(uri.getScheme())) {
					b.setIconUri(PermataContentProvider.toImgUri(uri));
	...
	interface StreamItem extends PlayableItem, BrowsableItem {
		String STREAM_START_TIME = "my.app.media.stream.START_TIME";
		String STREAM_END_TIME = "my.app.media.stream.END_TIME";
	...
	interface BrowsableItem extends Item {
		...
		@Override
		default int getIcon() {
			return my.app.utils.R.drawable.folder;
		}
