...
import static my.app.permata.ui.activity.MainActivityDelegate.INTENT_ACTION_OPEN;
import static my.app.permata.ui.activity.MainActivityDelegate.INTENT_ACTION_PLAY;
...
import static my.app.permata.ui.activity.MainActivityDelegate.toIntentUri;
import static my.app.utils.async.Completed.completed;
import static my.app.utils.async.Completed.completedNull;
import static my.app.utils.collection.CollectionUtils.forEach;
import static my.app.utils.collection.CollectionUtils.putIfAbsent;
import static my.app.utils.misc.MiscUtils.ifNull;
...
public class AtvInterface implements Item.ChangeListener {
	...
	private void updateProg(Prog p) {
		...
		p.loading = getDescription(p.item).main().onSuccess(d -> {
			...
			if (art != null) b.setPosterArtUri(PermataContentProvider.toImgUri(art));
	...
	private FutureSupplier<Uri> createIcon(Item item) {
		...
		return lib.getBitmapCache().addImage(uri, () -> {
			...
			if (drawBg) {
				Drawable bg = ResourcesCompat.getDrawable(res, R.drawable.permata_bg, theme);
